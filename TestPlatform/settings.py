...
SECRET_KEY = '56sr-ji5g6zu6+1ks!)&5t6&ns&v*#=11ic8nyecuyl=t&22y&'
...
AUTH_USER_MODEL = 'APICloud.UserInfo'
...
MEDIA_URL = "/media/"
MEDIA_ROOT = os.path.join(BASE_DIR, "media")    # urls.py 文件中也要进行配置


LOGIN_URL = '/login/'   # 认证装饰器的配置
...
LOGGING = {
    ...
    'handlers': {
        ...
        'error': {
            'level': 'ERROR',
            'class': 'logging.handlers.RotatingFileHandler',  # 保存到文件，自动切
            'filename': os.path.join(BASE_LOG_DIR, "log_err.log"),  # 日志文件
        ...
        'collect': {
            'level': 'INFO',
            'class': 'logging.handlers.RotatingFileHandler',  # 保存到文件，自动切
            'filename': os.path.join(BASE_LOG_DIR, "log_collect.log"),
            'maxBytes': 1024 * 1024 * 50,  # 日志大小 50M
            'backupCount': 5,
            'formatter': 'collect',
            'encoding': "utf-8"
